...
def load(cat_name, 
     ...
     if not check_catalogue_exists(cat_file_name):
        ...
        if plot_data:
            ...
            if len(feats) == 1:
                disp.histogram(data=df, 
                          col=feats[0], 
                          title = "Histogram of "+ cat_name + " " + feats[0],
                          figsize=(8,6),
                          filename = plot_filename)
            elif len(feats) == 2:
                disp.scatter2D(data=df,
                                feats=feats,
                                figsize=(6,4),
...
def normalize_scores(df_scores = None):
    ...
    df_scores_normalized = df_scores_normalized[
        (df_scores_normalized.index >= 2) & (df_scores_normalized.index <= 10)
    ]

    df_scores_normalized["aic"] = normalize(
        df_scores_normalized["aic"].values, inverse=True
    )
    df_scores_normalized["bic"] = normalize(
        df_scores_normalized["bic"].values, inverse=True
    )
    df_scores_normalized["wcss"] = normalize(
        df_scores_normalized["wcss"].values, inverse=True
    )
    ...
    df_scores_normalized["dbs"] = normalize(
        df_scores_normalized["dbs"].values, inverse=True
    )
