...
cross_comparison_metrics = {
    "wasser": {
        "description": "Wasserstein Distance",
        "func": lambda X1, X2: stats.wasserstein_distance_bootstrap(X1, X2),
    },
    "js": {
        "description": "Jensen-Shannon Distance",
        "func": lambda X1, X2: stats.jensen_shannon_distance_bootstrap(X1, X2),
    },
}
...
def get_comparison_metrics(
    ...
    """
    Retrieves a list of available cross-catalogue comparison metrics.
    Optionally logs a human-readable description of each metric.
    ...
    """
    if metric_names is None:
        metric_list = list(cross_comparison_metrics.keys())
    else:
        metric_list = metric_names

    # get metrics
    metrics = {
        metric: cross_comparison_metrics[metric]
        for metric in metric_list
        if metric in cross_comparison_metrics
    }

    if verbose:
        logger.info("Available metrics:")
        for mm in metric_list:
            logger.info(f"  > {mm}: {cross_comparison_metrics[mm]['description']}")
